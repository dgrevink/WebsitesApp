    ...
    */
    function buildMessage($params = array())
    {
        ...
        if (isset($message)) {
            ...
            $message_id = sprintf('<%s.%s@%s>', base_convert(time(), 10, 36), base_convert(rand(), 10, 36), isset($GLOBALS['HTTP_SERVER_VARS']['HTTP_HOST']) ? $GLOBALS['HTTP_SERVER_VARS']['HTTP_HOST'] : '');
        ...
        switch ($type) {
            case 'mail':
                ...
                if (!empty($this->return_path)) {
	                $headers[] = 'Return-Path: ' . $this->_encodeHeader($this->return_path, $this->build_params['head_charset']);
	            }
