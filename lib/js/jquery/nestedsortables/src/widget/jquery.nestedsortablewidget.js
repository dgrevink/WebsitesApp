	...
	userWarning : function(e, message) {
		var sel = jQuery('.' + e.nestedSortWidgetCfg.classes.warning, e)
			...
			.fadeIn(
				'slow',
				function() {
					setTimeout(function() {sel.fadeOut('slow');}, '10000');
				}
			);
	...
	*/
	...
	buildAndShowList:  function (e, page){
		...
		var buildSaveAndProgress = function () {
			var classes = e.nestedSortWidgetCfg.classes;
			var text = e.nestedSortWidgetCfg.text;
			var saveButton = jQuery("<input type='submit' class='"+classes.disabledSaveButton+"' style='display: none;' value='"+text.saveButton+"'/>");
	...
	save: function() {
		...
		var onSuccess = function(e, returnText) {
			...
			$('#status').html("<span class='ok'>" + e.nestedSortWidgetCfg.text.saveMessage + "</span>").fadeIn(1).fadeTo(5000, 1).fadeOut(1000);

			
			jQuery('.' + e.nestedSortWidgetCfg.classes.saveButton)
				.removeClass(e.nestedSortWidgetCfg.classes.saveButton)
				.addClass(e.nestedSortWidgetCfg.classes.disabledSaveButton);

			jQuery('img.drag-handle').css('display', 'none');
			jQuery('input.nsw-disabled-save-button').css('display', 'none'); 
		...
		return this.each(
			function() {
				...
				this.nestedSortWidgetCfg = {
					...
					whiteMargin: conf.whiteMargin ? conf.whiteMargin : 0,
