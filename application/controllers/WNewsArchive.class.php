...
class WNewsArchive extends WSController {
	function index( $params = null ) {
		...
		if (!$this->_check_init()) {
			return "<div class='ws-debug'>WNewsArchive ( Nouvelles - Archives par année ): BAD DATABASE STRUCTURE</div>";
		}
		...
		$temp = MyActiveRecord::FindAll('news', "language = '" . $this->language . "' and ( year(ddate) = " . $year . " ) ", "ddate desc", $max);
		$records = array();
		foreach($temp as $r) {
			if ($this->language == 'fr') {
