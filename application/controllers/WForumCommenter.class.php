...
/**
 *	RNAME: Forum - Commentaires sur la page
 *	PATH: 
 *	NOTE: Affiche un commenteur de page.
 *	VERSION: 1.0
 *	ACTIVE: NO
 *
 */
...
/* notes:
	- datetime on comment post
	- full ajax
	- login/password procedure
	- inspiration: blogger/the verge
*/

class WForumCommenter extends WSController {
	function index( $params = null ) {
		...
		$thread = MyActiveRecord::FindFirst('forumthreads', "contents_id = {$this->params['infos']['id']}");
		...
		// Show all comment on thread if it exists
		$thread = MyActiveRecord::FindFirst('forumthreads', "contents_id = {$this->params['infos']['id']} and language = '{$this->params['language']}'");
		if ($thread) {
