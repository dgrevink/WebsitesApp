...
<h3>Tell someone about this page...</h3>
<form id='mailpage' name='mailpage'>
<p><label>Your email:<br/><input type='text' name='emailfrom' id='formemailfrom' class='ws-mailpage-text' /></label></p>
<p><label>Destination email:<br/><input type='text' name='emailto' id='formemailto' class='ws-mailpage-text' /></label></p>
<p><label>Subject:<br/><input type='text' name='subject' id='formsubject' class='ws-mailpage-text' /></label></p>
<p><label>Message:<br/><textarea name='content' id='formcontent' class='ws-mailpage-content'>Hi there,
...
$(document).ready(function(){
	...
	$('#mailpage').live('submit', function() {
		$.post('/WProduct/sendpage/', {
			...
			function(data) {
				if (data != 'OK') {
					alert('Please fill in the information carefully.');
				}
				else {
					$('.modalBoxBodyContent').html("<span class='endmessage'>Message has been sent !</span><input id='mailpageclose' type='button' value='Close' />");
	  				$('.result').html(data);
				}
			}
		);
